<div id="block-blog-admin">
	...
	<div id="blog-admin-header" class="block-admin-subheading">
		<button type="button" style="float:right" name="blog-admin-close"
			onclick="window.location='administration.php?story-blog-admin=false';">
		CLOSE
		</button>
		<h4 style="margin:0px; padding:10px 0px 0px 0px;">Story Blog Administration: Comments</h4>
	</div>
	...
	<div class="content">
		...
		<table>
			...
			<?php
				$comments = Comment::selectCommentsByCollection('story', 'date');
				...
				for($i=0; $i<count($comments); $i++) {
					...
					if($status == 'approved') {
						echo '<tr id="comment-' . $id . '" onclick="window.location=\'story.php#comment-' . $id .'\';">';
					} else {
						...
						echo '<td class="center">';
							...
							$action = 'administration.php?story-blog-admin=true';
